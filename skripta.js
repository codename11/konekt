	...
	$("#forma2").submit(function(event){
		...
		$.ajax({
			...
			success:function(result){
				...
				for (let i=0;i<len;i++){
					table += "<tr><td class='text-center'>"+i+". <input type='checkbox' style='display:none;' value='"+obj.id[i]+"'></td><td class='text-center'>"+obj.ime[i]+"</td><td class='text-center'>"+obj.prezime[i]+"</td><td class='text-center'>"+obj.datum_rođenja[i]+"</td><td class='text-center'>"+obj.adresa[i]+"</td><td class='text-center'>"+obj.mesto[i]+"</td><td class='text-center'>"+obj.kompanija[i]+"</td><td class='text-center'>"+obj.broj[i]+"</td><td><button id='br"+i+"' class='btn btn-default btn-xs' onclick='populateModal(this)' data-toggle='modal' data-target='#myModal' style='float: left;'><span class='glyphicon glyphicon-check' title='Edit record'></span></button><button class='btn btn-default btn-xs' style='float: right;' id='"+obj.id[i]+"' onclick='delRecord(this)'><span class='glyphicon glyphicon-remove'  style='color:red;' title='Remove record'></span></button></td>";	
				}
